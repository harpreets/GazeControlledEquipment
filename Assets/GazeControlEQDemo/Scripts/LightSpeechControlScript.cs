...
public class LightSpeechControlScript : MonoBehaviour
{
    HueComm lightingControl;
    public AreaHitDetector areaHitDetector;
    ...
    void Start()
    {
        lightingControl = GetComponent<HueComm>();
    ...
    void FixedUpdate()
    {
        ...
        if (speechManager != null && speechManager.IsSapiInitialized())
        {
            if (speechManager.IsPhraseRecognized())
            {
                ...
                {
                    case "ON":
                        lightingControl.SendOnMessageToLight(areaHitDetector.currentActiveLightArea);
                        break;
                    case "OFF":
                        lightingControl.SendOffMessageToLight(areaHitDetector.currentActiveLightArea);
                        break;
                    case "LEFT":
                        walkDirection = -0.2f;
                        if (walkSpeed == 0f)
